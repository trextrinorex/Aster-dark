...
const earthMat = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    uniform sampler2D dayTexture; uniform sampler2D nightTexture; uniform sampler2D topoTexture;
    uniform vec3 sunDirection; uniform float nightIntensity; uniform float time;
...
const atmos = new THREE.Mesh(
  ...
  new THREE.ShaderMaterial({
    ...
    uniforms: { uColor: { value: new THREE.Color(0x5a9bb0) }, uPower: { value: 0.32 } },
    vertexShader: `varying vec3 vN; varying vec3 vV; void main() { vec4 mv = modelViewMatrix * vec4(position, 1.0); vV = normalize(-mv.xyz); vN = normalize(normalMatrix * normal); gl_Position = projectionMatrix * mv; }`,
    fragmentShader: `uniform vec3 uColor; uniform float uPower; varying vec3 vN; varying vec3 vV; void main() { float r = pow(1.0 - max(dot(vN, vV), 0.0), 3.4); gl_FragColor = vec4(uColor, r * uPower); }`
  })
);
...
const clouds = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({ map: cloudMap, transparent: true, opacity: 0.14, depthWrite: false, blending: THREE.AdditiveBlending })
);
...
cityData.forEach(([name, lat, lon, code, index]) => {
  ...
  const mesh = new THREE.Mesh(new THREE.SphereGeometry(size, 12, 12), new THREE.MeshBasicMaterial({ color: 0xffc266, transparent: true, opacity: 0.9, depthWrite: false }));
  mesh.position.copy(latLon(lat, lon));
  mesh.userData = { name, lat, lon, code, index };
  earthGroup.add(mesh);
  cityPoints.push(mesh);
  const glow = new THREE.Mesh(new THREE.SphereGeometry(size * 3.2, 10, 10), new THREE.MeshBasicMaterial({ color: 0xff9933, transparent: true, opacity: 0.22, blending: THREE.AdditiveBlending, depthWrite: false }));
...
function onPointerMove(e) {
  ...
  lx = e.clientX;
  ly = e.clientY;
  // Full-screen swipe ≈ ~2 rotations; touch is extra fast
  const sensX = (Math.PI * (isTouch ? 4.5 : 2.4)) / Math.max(innerWidth, 320);
  const sensY = (Math.PI * (isTouch ? 2.5 : 1.3)) / Math.max(innerHeight, 320);
  tryy += dx * sensX;
  trx += dy * sensY;
  trx = Math.max(-1.0, Math.min(1.0, trx));
  rx = trx;
  ry = tryy;
}
...
canvas.addEventListener('touchmove', e => {
  ...
  lx = t.clientX;
  ly = t.clientY;
  const sensX = (Math.PI * 4.5) / Math.max(innerWidth, 320);
  const sensY = (Math.PI * 2.5) / Math.max(innerHeight, 320);
  tryy += dx * sensX;
  trx += dy * sensY;
  trx = Math.max(-1.0, Math.min(1.0, trx));
  rx = trx;
...
function animate() {
  ...
  if (!matchMedia('(prefers-reduced-motion: reduce)').matches) {
    ...
    if (drag) { rx = trx; ry = tryy; }
    else { rx += (trx - rx) * 0.12; ry += (tryy - ry) * 0.12; }
    scale += (ts - scale) * 0.1;
...
function nearest(y) {
  return Object.keys(history).map(Number).reduce((a, b) => Math.abs(b - y) < Math.abs(a - y) ? b : a);
}
...
function focusRegion(n) {
  ...
  const f = { INDIA: [0.18, -0.85], EUROPE: [0.48, -0.12], 'NORTH AMERICA': [0.22, 1.12], 'EAST ASIA': [0.2, -2.28], AFRICA: [-0.1, -0.42], GLOBAL: [0.12, -0.82] }[n];
  trx = f[0]; tryy = f[1]; ts = n === 'GLOBAL' ? 1 : 1.2;
  autoRotate = false; resetIdle();
...
cityData.forEach(([name, lat, lon, code, index]) => {
  ...
  b.addEventListener('click', () => {
    ...
    document.querySelector('#cityCoords').textContent = `${Math.abs(lat).toFixed(4)}° ${lat >= 0 ? 'N' : 'S'} / ${Math.abs(lon).toFixed(4)}° ${lon >= 0 ? 'E' : 'W'}`;
...
function pollutionUpdate() {
  ...
  document.querySelector('#skyMode').textContent = v < 20 ? 'NATURAL NIGHT' : v < 65 ? 'SUBURBAN SKY' : 'CITY SKY';
}
...
function cityUpdate() {
  ...
  document.querySelector('#restoreState').textContent = v === 0 ? 'NATURAL NIGHT' : v < 40 ? 'REDUCED LIGHTING' : 'ARTIFICIAL LIGHT';
}
...
addEventListener('keydown', e => {
  ...
  if (e.key === 'ArrowRight') { yearSlider.value = Math.min(2026, +yearSlider.value + 2); yearUpdate(); }
  if (e.key === 'ArrowLeft') { yearSlider.value = Math.max(1992, +yearSlider.value - 2); yearUpdate(); }
});
